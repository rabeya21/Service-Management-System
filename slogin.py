...
from socialregistration import Sregistration
...
class SLogin_Window():
    ...
    def login(self):
        if self.txtuser.get() == "mash" and self.txtpass.get() == "1234":
            ...
            my_cursor.execute("select * from register where email=%s and passw=%s",(self.var_email.get(),self.var_password.get()))
            row=my_cursor.fetchone()
            if row!=None:
                messagebox.showerror("Error","Invalied Username and Password",parent=self.root)
            else:
                open_main=messagebox.askyesno("YseNo","Access only Social Holder",parent=self.root)
                if open_main > 0:
    ...
    def reset(self):
        if self.cmb_ide.get()=="select":
            ...
            conn = mysql.connector.connect( host="127.0.0.1", user="root", password="", database="socialregistration" )
            my_cursor = conn.cursor()
            qury=("select * from register where email=%s and ide=%s and id=%s")
    ...
    def registerpage(self):
        self.new_window = Toplevel( self.root )
        self.app = Sregistration( self.new_window )
