...
from dregistration import Dregistration
...
class Login_Window():
    ...
    def __init__(self, root) :
        ...
        # **************Labels**************
        self.username = lbl = Label(login_frame, text="User Name", font=("times in roman", 10, "bold"), fg="white", bg="black")
    ...
    def login(self):
        if self.txtuser.get() == "mash" and self.txtpass.get() == "1234":
            ...
            my_cursor.execute("select * from dregister where Mail=%s and pass=%s",(self.var_email.get(),self.var_password.get()))
            row=my_cursor.fetchone()
            if row!=None:
                messagebox.showerror("Error","Invalied Username and Password",parent=self.root)
            else:
                open_main=messagebox.askyesno("YseNo","Access only Doctor",parent=self.root)
                if open_main > 0:
    ...
    def reset(self):
        if self.cmb_ide.get()=="select":
            ...
            conn = mysql.connector.connect( host="127.0.0.1", user="root", password="", database="doctorinfo" )
            my_cursor = conn.cursor()
            qury=("select * from dregister where Mail=%s and idtype=%s and id=%s")
            ...
            if row==None:
                ...
                query=("update dregister set pass=%s where Mail=%s")
