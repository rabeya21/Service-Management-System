...
class Dregistration:
    def __init__(self, root):
        ...
        cmb_ide['values'] = ("Select", "National Id", "Hospital Id", "Passport Id")
        ...
        self.doctor = Label(frame1, text="Type Of Doctor", font=("times new roman", 15, "bold"), bg="white",fg="black").place( x=305, y=250)

        cmb_doc = ttk.Combobox(frame1,textvariable=self.var_dtype, font=("times new roman", 13), state="readonly", justify=CENTER)
        cmb_doc['values'] = ("Select", "MEDICINE", "NEUROLOGISTS", "PEDIATRICIANS", "CARDIOLOGISTS", "SURGEONS", "PHYSIATRIST")
        ...
        self.timS = Label(frame1, text="Chamber Opening Time", font=("times new roman", 15, "bold"), bg="white",
                    fg="black").place(x=305,
                                      y=390)
        self.txt_timS = Entry(frame1,textvariable=self.var_TimeS, font=("times new roman", 15), bg="gray")
        self.txt_timS.place(x=305, y=420, width=200)
        ...
        self.timE = Label(frame2, text="Chamber Ending Time", font=("times new roman", 15, "bold"), bg="white", fg="black").place(
            x=540,
            y=10)
        self.txt_timE = Entry(frame2, textvariable=self.var_TimeE, font=("times new roman", 15), bg="gray")
        self.txt_timE.place(x=540, y=40, width=200)

        self.pay = Label(frame2, text="Fees Required", font=("times new roman", 15, "bold"), bg="white", fg="black").place(
            x=835,
            y=10)
        self.txt_pay = Entry(frame2,textvariable=self.var_fees, font=("times new roman", 15), bg="gray")
        self.txt_pay.place(x=835, y=40, width=200)
    ...
    def register_data(self):
        if self.txt_fname.get() == "" or self.txt_email.get() == "" or self.txt_contact.get() == "" or self.txt_passw.get() == "" or self.txt_cnfrm.get() == "":
            ...
            conn=mysql.connector.connect(host="127.0.0.1", user="root", password="",database="doctorinfo" )
            my_cursor=conn.cursor()
            query=("select * from dregister where Mail=%s")
            ...
            if row!=None:
                ...
                my_cursor.execute("insert into dregister values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(self.ver_idtype.get(),
                                                                                                          self.ver_ids.get(),
                                                                                                          self.var_name.get(),
                                                                                                          self.var_email.get(),
                                                                                                          self.var_mobail.get(),
    ...
    def next(self):
        ...
        self.app = Dregistration(self.new_window)
